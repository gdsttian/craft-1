...
INPUTDIR="$CRAFTROOT/train-and-devel-standoff-separated-deduplicated"

OUTPUTDIR="$CRAFTROOT/train-and-devel-standoff-separated-deduplicated-deoverlapped"
...
for d in `find "$INPUTDIR"/* -type d`; do
	...
	python "$SCRIPTDIR/remove-overlapped.py" "$f" > "$od"/`basename $f`
    done
done
